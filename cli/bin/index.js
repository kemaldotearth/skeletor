...
var createPackageJson = (libName, includeTailwind, includeStyledComponents, includeStorybook) => {
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        ...
        scripts: {
          build: "rimraf dist && tsc --emitDeclarationOnly && NODE_ENV=production rollup --config",
          tsc: "tsc"
        },
        author: "skeletor-cli",
        license: "UNLICENSED",
        devDependencies: __spreadValues(__spreadValues(__spreadValues({
          "@babel/core": "^7.14.3",
...
var createFolderStructure = (libName, includeTailwind, includeStorybook) => {
...
var createClassNamesUtil = (libName) => {
  fs.writeFileSync(
...
var createRollupConfig = (libName, includeTailwind) => {
  fs.writeFileSync(
...
var createTailwindConfig = (libName) => {
  fs.writeFileSync(
...
var createPostCssConfig = (libName) => {
  fs.writeFileSync(
...
var createStoryBookConfig = (libName, includeTailwind) => {
  fs.writeFileSync(
  ...
  constructor() {
    ...
    this.#ourEmit = function(event, data, ...args) {
      ...
      if (self.#requests > 0 || stdin.emit === self.#ourEmit) {
        ...
        Reflect.apply(import_node_process6.default.stdin.emit, this, [event, data, ...args]);
      }
    };
  }
  start() {
...
program2.command("generate:ui").option("--with-tailwind", "Adds Tailwind CSS to your package.").option("--with-styled-components", "Adds Styled Components to your package.").option("--with-storybook", "Adds Storybook to your package.").action((options) => __async(exports, null, function* () {
  ...
  updateSpinnerText("\u{1F477}\u200D\u2640\uFE0F Crafting your first components...");
  ...
  updateSpinnerText("\u{1F9FB} Adding a rollup.config.js...");
  yield new Promise((resolve) => setTimeout(resolve, 2e3));
  createRollupConfig(libName, includeTailwind);
  if (includeTailwind) {
    updateSpinnerText("\u{1F4A8} Setting up TailwindCSS...");
    yield new Promise((resolve) => setTimeout(resolve, 4e3));
  ...
  if (includeStorybook) {
    updateSpinnerText("\u{1F4DA} Setting up Storybook...");
    yield new Promise((resolve) => setTimeout(resolve, 4e3));
  ...
  updateSpinnerText(`\u{1F480} That's it! ~ cd ${libName} && npm install`);
  yield new Promise((resolve) => setTimeout(resolve, 2e3));
  spinnerSuccess();
}));
